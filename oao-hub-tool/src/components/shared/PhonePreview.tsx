...
function CardView({ config }: { config: MainConfiguration }) {
  ...
  return (
    <div className="px-3 py-4 space-y-3">
      {/* Status bar */}
      <p className="text-[9px] text-ink-300 text-center">Trang 1 — Card trong list</p>
      ...
      {/* Freeze Banner preview */}
      {config.freeze_banner.enabled && (
        <div className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 shadow-sm border border-surface-100">
          <div className="w-8 h-8 rounded-lg bg-surface-100 shrink-0 overflow-hidden">
          ...
          <span className="text-[8px] bg-red-500 text-white px-1.5 py-0.5 rounded-full shrink-0">Đang freeze</span>
        </div>
      )}

      <p className="text-[8px] text-ink-300 text-center pt-1">Freeze Banner xuất hiện khi user scroll</p>
    </div>
  )
}
...
export function PhonePreview({ config }: PhonePreviewProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 select-none">
      ...
      <p className="text-[10px] text-ink-300 text-center">Preview chỉ mang tính minh hoạ</p>
    </div>
  )
}
